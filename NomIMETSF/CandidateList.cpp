...
CCandidateList::CCandidateList(com_ptr<NomIMETSF> pTextService)
...
void CCandidateList::UpdateUI(const Context& ctx, const Status& status) {
  if (_ui->style().inline_preedit) {
    _ui->style().client_caps |= nomime::INLINE_PREEDIT_CAPABLE;
  } else {
    _ui->style().client_caps &= ~nomime::INLINE_PREEDIT_CAPABLE;
  }
...
void NomIMETSF::_UpdateUI(const Context& ctx, const Status& status) {
  _cand->UpdateUI(ctx, status);
}

void NomIMETSF::_StartUI() {
  _cand->StartUI();
}

void NomIMETSF::_EndUI() {
  _cand->EndUI();
}

void NomIMETSF::_ShowUI() {
  _cand->Show(TRUE);
}

void NomIMETSF::_HideUI() {
  _cand->Show(FALSE);
}

com_ptr<ITfContext> NomIMETSF::_GetUIContextDocument() {
  return _cand->GetContextDocument();
}

void NomIMETSF::_DeleteCandidateList() {
  _cand->Destroy();
}

void NomIMETSF::_SelectCandidateOnCurrentPage(size_t index) {
...
void NomIMETSF::_HandleMousePageEvent(bool* const nextPage,
...
void NomIMETSF::_HandleMouseHoverEvent(const size_t index) {
...
void NomIMETSF::HandleUICallback(size_t* const sel,
                                 size_t* const hov,
                                 bool* const next,
