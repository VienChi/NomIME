...
#define CHECK_HR(hr) \
  ...
  BOOL viEnable = (profile == L"vi");
  // fallback hans enable
  hansEnable = hansEnable || (!viEnable && !hansEnable);

  const auto text_service_desc = get_nomime_ime_name();
  ...
  CHECK_HR(register_profile(TEXTSERVICE_LANGID_VIETNAM, NULL, viEnable));
  // NomIMEIME not support these languages, so HKL is NULL and they are only
  // available as optional keyboards (not auto-enabled at install time).
  CHECK_HR(register_profile(TEXTSERVICE_LANGID_HANT, NULL, false));
...
void UnregisterProfiles() {
  ...
  unregister_profile(TEXTSERVICE_LANGID_VIETNAM);
}
  ...
  if (fRet = RegCreateKeyExA(HKEY_CLASSES_ROOT, achIMEKey, 0, NULL,
    ...
    if (fRet &=
...
#ifdef _M_ARM64
      {
        ...
        // When loaded, nomime.dll will be redirected to nomimeARM64.dll on
        // ARM64 processes, and nomimex64.dll on x64 processes.
        //
        // But GetModuleFileNameA will return the actual loaded DLL name aka
        // nomimeARM64.dll Rewrite the path to point to the redirector.
