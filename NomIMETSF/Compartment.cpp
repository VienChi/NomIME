...
BOOL NomIMETSF::_IsKeyboardDisabled() {
...
BOOL NomIMETSF::_IsKeyboardOpen() {
...
HRESULT NomIMETSF::_SetKeyboardOpen(BOOL fOpen) {
  HRESULT hr = E_FAIL;
  com_ptr<ITfCompartmentMgr> pCompMgr;
...
HRESULT NomIMETSF::_GetCompartmentDWORD(DWORD& value, const GUID guid) {
  HRESULT hr = E_FAIL;
...
HRESULT NomIMETSF::_SetCompartmentDWORD(const DWORD& value, const GUID guid) {
  HRESULT hr = S_OK;
  com_ptr<ITfCompartmentMgr> pComMgr;
  if (_pThreadMgr->QueryInterface(&pComMgr) == S_OK) {
    ITfCompartment* pCompartment;
...
BOOL NomIMETSF::_InitCompartment() {
  using namespace std::placeholders;

  auto callback = std::bind(&NomIMETSF::_HandleCompartment, this, _1);
...
void NomIMETSF::_UninitCompartment() {
  if (_pKeyboardCompartmentSink) {
...
HRESULT NomIMETSF::_HandleCompartment(REFGUID guidCompartment) {
  if (IsEqualGUID(guidCompartment, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE)) {
    if (_isToOpenClose) {
      ...
      _HandleLangBarMenuSelect(_status.ascii_mode
                                   ? ID_NOMIMETRAY_ENABLE_ASCII
                                   : ID_NOMIMETRAY_DISABLE_ASCII);
    ...
    BOOL isOpen = _IsKeyboardOpen();
    if (isOpen) {
      nomime::ResponseParser parser(NULL, NULL, &_status, NULL,
                                    &_cand->style());
