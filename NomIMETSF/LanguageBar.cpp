...
static LPCWSTR GetNomIMERegName() {
  LPCWSTR NOMIME_REG_NAME_;
  if (is_wow64())
    NOMIME_REG_NAME_ = L"Software\\WOW6432Node\\SinoNom\\NomIME";
  else
    NOMIME_REG_NAME_ = L"Software\\SinoNom\\NomIME";

  return NOMIME_REG_NAME_;
}
...
CLangBarItemButton::CLangBarItemButton(com_ptr<NomIMETSF> pTextService,
                                       REFGUID guid,
                                       nomime::UIStyle& style)
    : _status(0),
      _style(style),
...
STDAPI CLangBarItemButton::GetInfo(TF_LANGBARITEMINFO* pInfo) {
  ...
  pInfo->ulSort = 1;
  lstrcpyW(pInfo->szDescription, L"NomIMETSF Button");
  return S_OK;
}
...
STDAPI CLangBarItemButton::GetTooltipString(BSTR* pbstrToolTip) {
  *pbstrToolTip = SysAllocString(
      L"Nhấp trái để chuyển chế độ\n\nNhấp phải để xem thêm tùy chọn");
...
STDAPI CLangBarItemButton::OnClick(TfLBIClick click,
  ...
  if (click == TF_LBI_CLK_LEFT) {
    _pTextService->_HandleLangBarMenuSelect(
        ascii_mode ? ID_NOMIMETRAY_DISABLE_ASCII : ID_NOMIMETRAY_ENABLE_ASCII);
...
STDAPI CLangBarItemButton::GetText(BSTR* pbstrText) {
  *pbstrText = SysAllocString(L"NomIMETSF Button");
...
void CLangBarItemButton::UpdateNomIMEStatus(nomime::Status stat) {
...
std::wstring NomIMETSF::_GetRootDir() {
  std::wstring dir{};
  RegGetStringValue(HKEY_LOCAL_MACHINE, GetNomIMERegName(), L"NomIMERoot", dir);
  return dir;
}

void NomIMETSF::_HandleLangBarMenuSelect(UINT wID) {
  std::wstring dir{};
  switch (wID) {
    case ID_NOMIMETRAY_RERUN_SERVICE:
    case ID_NOMIMETRAY_INSTALLDIR:
      if (RegGetStringValue(HKEY_LOCAL_MACHINE, GetNomIMERegName(),
                            L"NomIMERoot", dir) == ERROR_SUCCESS) {
        if (wID == ID_NOMIMETRAY_RERUN_SERVICE) {
          std::thread th([dir]() {
      ...
      break;
    case ID_NOMIMETRAY_USERCONFIG:
      if (FAILED(RegGetStringValue(HKEY_CURRENT_USER, L"Software\\SinoNom\\NomIME",
                                   L"RimeUserDir", dir)) ||
          dir.empty()) {
        WCHAR _path[MAX_PATH] = {0};
        ExpandEnvironmentStringsW(L"%AppData%\\NomIME", _path, _countof(_path));
...
HWND NomIMETSF::_GetFocusedContextWindow() {
  HWND hwnd = NULL;
...
BOOL NomIMETSF::_InitLanguageBar() {
...
void NomIMETSF::_UninitLanguageBar() {
  com_ptr<ITfLangBarItemMgr> pLangBarItemMgr;
...
void NomIMETSF::_UpdateLanguageBar(nomime::Status stat) {
  if (!_pLangBarButton)
    return;
  DWORD flags;
  ...
  _pLangBarButton->UpdateNomIMEStatus(stat);
}

void NomIMETSF::_ShowLanguageBar(BOOL show) {
  if (!_pLangBarButton)
    return;
  _pLangBarButton->Show(show);
}

void NomIMETSF::_EnableLanguageBar(BOOL enable) {
  if (!_pLangBarButton)
    return;
