...
class CStartCompositionEditSession : public CEditSession {
 public:
  CStartCompositionEditSession(com_ptr<NomIMETSF> pTextService,
...
STDAPI CStartCompositionEditSession::DoEditSession(TfEditCookie ec) {
  ...
  if ((pContextComposition->StartComposition(
     ...
     *   See https://github.com/rime/nomime/pull/883#issuecomment-1567625762
     */
...
void NomIMETSF::_StartComposition(com_ptr<ITfContext> pContext,
                                  BOOL fCUASWorkaroundEnabled) {
...
class CEndCompositionEditSession : public CEditSession {
 public:
  CEndCompositionEditSession(com_ptr<NomIMETSF> pTextService,
                             com_ptr<ITfContext> pContext,
                             com_ptr<ITfComposition> pComposition,
                             BOOL clear = TRUE)
...
void NomIMETSF::_EndComposition(com_ptr<ITfContext> pContext, BOOL clear) {
...
class CGetTextExtentEditSession : public CEditSession {
 public:
  CGetTextExtentEditSession(com_ptr<NomIMETSF> pTextService,
                            com_ptr<ITfContext> pContext,
...
BOOL NomIMETSF::_UpdateCompositionWindow(com_ptr<ITfContext> pContext) {
  com_ptr<ITfContextView> pContextView;
...
void NomIMETSF::_SetCompositionPosition(const RECT& rc) {
...
class CInlinePreeditEditSession : public CEditSession {
 public:
  CInlinePreeditEditSession(com_ptr<NomIMETSF> pTextService,
                            com_ptr<ITfContext> pContext,
                            com_ptr<ITfComposition> pComposition,
                            const std::shared_ptr<nomime::Context> context)
      : CEditSession(pTextService, pContext),
        _pComposition(pComposition),
        _context(context) {}
 ...
 private:
  com_ptr<ITfComposition> _pComposition;
  const std::shared_ptr<nomime::Context> _context;
};
...
STDAPI CInlinePreeditEditSession::DoEditSession(TfEditCookie ec) {
  ...
  for (size_t i = 0; i < _context->preedit.attributes.size(); i++) {
    if (_context->preedit.attributes.at(i).type == nomime::HIGHLIGHTED) {
...
BOOL NomIMETSF::_ShowInlinePreedit(
    com_ptr<ITfContext> pContext,
    const std::shared_ptr<nomime::Context> context) {
...
class CInsertTextEditSession : public CEditSession {
 public:
  CInsertTextEditSession(com_ptr<NomIMETSF> pTextService,
                         com_ptr<ITfContext> pContext,
                         com_ptr<ITfComposition> pComposition,
...
BOOL NomIMETSF::_InsertText(com_ptr<ITfContext> pContext,
                            const std::wstring& text) {
...
void NomIMETSF::_UpdateComposition(com_ptr<ITfContext> pContext) {
  HRESULT hr;
...
/* Composition State */
STDAPI NomIMETSF::OnCompositionTerminated(TfEditCookie ecWrite,
...
void NomIMETSF::_AbortComposition(bool clear) {
...
void NomIMETSF::_FinalizeComposition() {
  _pComposition = nullptr;
}

void NomIMETSF::_SetComposition(com_ptr<ITfComposition> pComposition) {
  _pComposition = pComposition;
}

BOOL NomIMETSF::_IsComposing() {
