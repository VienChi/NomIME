// NomIMEDeployer.cpp : Defines the entry point for the application.
//
#include "stdafx.h"
...
static int Run(LPTSTR lpCmdLine) {
  ...
  if (!wcscmp(L"/?", lpCmdLine) || !wcscmp(L"/help", lpCmdLine)) {
    ...
    if (LoadString(GetModuleHandle(NULL), IDS_STR_HELP, msg,
      ...
      MessageBox(NULL,
                 L"Usage: NomIMEDeployer.exe [options]\n"
                 L"/? or /help		- Show this help message\n"
                 L"/deploy		- Update Workspace\n"
                 L"/dict		- Manage dictionary\n"
                 L"/sync		- Sync user data\n"
                 L"/install		- Install NomIME (Initial deployment)",
                 L"NomIME Deployer", MB_ICONINFORMATION | MB_OK);
    }
    return 0;
  }
