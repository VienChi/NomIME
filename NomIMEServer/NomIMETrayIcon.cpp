...
NomIMETrayIcon::NomIMETrayIcon(nomime::UI& ui)
    : m_style(ui.style()),
      m_status(ui.status()),
      m_mode(INITIAL),
      m_schema_zhung_icon(),
...
void NomIMETrayIcon::CustomizeMenu(HMENU hMenu) {}

BOOL NomIMETrayIcon::Create(HWND hTargetWnd) {
  HMODULE hModule = GetModuleHandle(NULL);
  CIcon icon;
  icon.LoadIconW(IDI_ZH);
  BOOL bRet =
      CSystemTray::Create(hModule, NULL, WM_NOMIME_TRAY_NOTIFY,
                          get_nomime_ime_name().c_str(), icon, IDR_MENU_POPUP);
  if (hTargetWnd) {
...
void NomIMETrayIcon::Refresh() {
  ...
  NomIMETrayMode mode = m_status.disabled     ? DISABLED
