#include "stdafx.h"
#include "NomIMEService.h"
#include "NomIMEServerApp.h"
#include <boost/thread.hpp>

NomIMEService* NomIMEService::_service = NULL;

NomIMEService::NomIMEService(BOOL fCanStop = TRUE,
...
NomIMEService::~NomIMEService() {
  if (_stoppedEvent != NULL) {
    CloseHandle(_stoppedEvent);
  }
}

BOOL NomIMEService::Run(NomIMEService& serv) {
  _service = &serv;
  SERVICE_TABLE_ENTRY serviceTable[] = {{NOMIME_SERVICE_NAME, ServiceMain},
                                        {NULL, NULL}};
...
void NomIMEService::Start(DWORD dwArgc = 0, PWSTR* pszArgv = NULL) {
  try {
...
void NomIMEService::ServiceMain(DWORD dwArgc, PWSTR* pszArgv) {
  _service->_statusHandle =
      RegisterServiceCtrlHandler(NOMIME_SERVICE_NAME, ServiceCtrlHandler);
...
void NomIMEService::ServiceCtrlHandler(DWORD dwCtrl) {
  switch (dwCtrl) {
...
void NomIMEService::SetServiceStatus(DWORD dwCurrentState,
