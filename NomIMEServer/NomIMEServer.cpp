// NomIMEServer.cpp : main source file for NomIMEServer.exe
//
...
int WINAPI _tWinMain(HINSTANCE hInstance,
  ...
  if (!wcscmp(L"/userdir", lpstrCmdLine)) {
    CreateDirectory(NomIMEUserDataPath().c_str(), NULL);
    NomIMEServerApp::explore(NomIMEUserDataPath());
    return 0;
  }
  if (!wcscmp(L"/nomimedir", lpstrCmdLine)) {
    NomIMEServerApp::explore(NomIMEServerApp::install_dir());
    return 0;
  }
  if (!wcscmp(L"/ascii", lpstrCmdLine) || !wcscmp(L"/nascii", lpstrCmdLine)) {
    ...
    {
      if (ascii)
        client.TrayCommand(ID_NOMIMETRAY_ENABLE_ASCII);
      else
        client.TrayCommand(ID_NOMIMETRAY_DISABLE_ASCII);
    }
    return 0;
  }
