...
if ($h -or $help -eq "--help") {
  ...
  -use [string]                  dev is for building nomime, nomime is for common usage in nomime(update rime.dll)
...
if ($PSBoundParameters.ContainsKey("use") -and ($use -eq "nomime" -or $use -eq "dev")) {
  $extract = $true
}
...
if ($os -eq "Windows" -and $use -eq "nomime") {
  if (Is64Bit) {
...
if ($null -ne $response.assets -and $response.assets.Count -gt 0) {
  ...
  if ($fileNames -and $cmdOk -and $extract) {
    ...
    if ($os -eq "Windows" -and $build_variant -eq "msvc") {
      ...
      function KillNomIMEServer {
        $processName = "NomIMEServer"
      ...
      if ( $use -eq "dev"){
        ...
        -and (Test-Path ".\output\Win32")) {
          ...
          Write-Host "❌ current directory is not a nomime source directory"
        }
      } elseif ($use -eq "nomime") {
        if ([Environment]::Is64BitOperatingSystem) {
          $registryPath = "HKLM:\SOFTWARE\WOW6432Node\Rime\NomIME"
        } else {
          $registryPath = "HKLM:\SOFTWARE\Rime\NomIME"
        }
        try {
          $nomimeRoot = (Get-ItemProperty -Path $registryPath -ErrorAction Stop).'NomIMERoot'
          $servercmd = Join-Path -Path $nomimeRoot -ChildPath "NomIMEServer.exe"
          KillNomIMEServer
          $processName = "NomIMEServer"
          $dllbit64 = Is64Bit
          MyCopyItem -src $(if ($dllbit64) { $dir64 } else { $dir86 }) -subpath "dist\lib\rime.dll" -dest $nomimeRoot
          MyCopyItem -src $(if ($dllbit64) { $dir64 } else { $dir86 }) -subpath "dist\lib\rime.pdb" -dest $nomimeRoot
          if ($dllbit64) {
            Remove-Item -Path $dir64 -Recurse -Force -Confirm:$false -ErrorAction SilentlyContinue
          } else {
            Remove-Item -Path $dir86 -Recurse -Force -Confirm:$false -ErrorAction SilentlyContinue
          }
          Start-Process $servercmd -WorkingDirectory $nomimeRoot
