...
LRESULT ServerImpl::OnCreate(UINT uMsg,
                             ...
                             BOOL& bHandled) {
  // not neccessary...
  ::SetWindowText(m_hWnd, NOMIME_IPC_WINDOW);
  return 0;
}
...
LRESULT ServerImpl::OnCommand(UINT uMsg,
  ...
  switch (uID) {
    case ID_NOMIMETRAY_ENABLE_ASCII:
      m_pRequestHandler->SetOption(lParam, "ascii_mode", true);
      return 0;
    case ID_NOMIMETRAY_DISABLE_ASCII:
...
DWORD ServerImpl::OnCommand(NOMIME_IPC_COMMAND uMsg,
                            DWORD wParam,
                            DWORD lParam) {
  BOOL handled = TRUE;
...
HWND ServerImpl::Start() {
  std::wstring instanceName = L"(NOMIME)Furandōru-Sukāretto-";
...
DWORD ServerImpl::OnEcho(NOMIME_IPC_COMMAND uMsg, DWORD wParam, DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
  return m_pRequestHandler->FindSession(lParam);
}

DWORD ServerImpl::OnStartSession(NOMIME_IPC_COMMAND uMsg,
                                 DWORD wParam,
                                 DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
...
DWORD ServerImpl::OnEndSession(NOMIME_IPC_COMMAND uMsg,
                               DWORD wParam,
                               DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
...
DWORD ServerImpl::OnKeyEvent(NOMIME_IPC_COMMAND uMsg,
                             DWORD wParam,
                             DWORD lParam) {
...
DWORD ServerImpl::OnShutdownServer(NOMIME_IPC_COMMAND uMsg,
                                   DWORD wParam,
                                   DWORD lParam) {
  Stop();
  return 0;
...
DWORD ServerImpl::OnFocusIn(NOMIME_IPC_COMMAND uMsg,
                            DWORD wParam,
                            DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
...
DWORD ServerImpl::OnFocusOut(NOMIME_IPC_COMMAND uMsg,
                             DWORD wParam,
                             DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
...
DWORD ServerImpl::OnUpdateInputPosition(NOMIME_IPC_COMMAND uMsg,
                                        DWORD wParam,
                                        DWORD lParam) {
  if (!m_pRequestHandler)
    return 0;
...
DWORD ServerImpl::OnStartMaintenance(NOMIME_IPC_COMMAND uMsg,
                                     DWORD wParam,
                                     DWORD lParam) {
  if (m_pRequestHandler)
...
DWORD ServerImpl::OnEndMaintenance(NOMIME_IPC_COMMAND uMsg,
                                   DWORD wParam,
                                   DWORD lParam) {
  if (m_pRequestHandler)
...
DWORD ServerImpl::OnCommitComposition(NOMIME_IPC_COMMAND uMsg,
                                      DWORD wParam,
                                      DWORD lParam) {
  if (m_pRequestHandler)
...
DWORD ServerImpl::OnClearComposition(NOMIME_IPC_COMMAND uMsg,
                                     DWORD wParam,
                                     DWORD lParam) {
  if (m_pRequestHandler)
...
DWORD ServerImpl::OnSelectCandidateOnCurrentPage(NOMIME_IPC_COMMAND uMsg,
                                                 DWORD wParam,
                                                 DWORD lParam) {
  if (m_pRequestHandler)
...
DWORD ServerImpl::OnHighlightCandidateOnCurrentPage(NOMIME_IPC_COMMAND uMsg,
                                                    DWORD wParam,
                                                    DWORD lParam) {
  if (m_pRequestHandler) {
    auto eat = [this](std::wstring& msg) -> bool {
...
DWORD ServerImpl::OnChangePage(NOMIME_IPC_COMMAND uMsg,
                               DWORD wParam,
                               DWORD lParam) {
  if (m_pRequestHandler) {
    auto eat = [this](std::wstring& msg) -> bool {
...
void ServerImpl::HandlePipeMessage(PipeMessage pipe_msg, _Resp resp) {
  ...
  PIPE_MSG_HANDLE(NOMIME_IPC_ECHO, OnEcho)
  PIPE_MSG_HANDLE(NOMIME_IPC_START_SESSION, OnStartSession)
  PIPE_MSG_HANDLE(NOMIME_IPC_END_SESSION, OnEndSession)
  PIPE_MSG_HANDLE(NOMIME_IPC_PROCESS_KEY_EVENT, OnKeyEvent)
  PIPE_MSG_HANDLE(NOMIME_IPC_SHUTDOWN_SERVER, OnShutdownServer)
  PIPE_MSG_HANDLE(NOMIME_IPC_FOCUS_IN, OnFocusIn)
  PIPE_MSG_HANDLE(NOMIME_IPC_FOCUS_OUT, OnFocusOut)
  PIPE_MSG_HANDLE(NOMIME_IPC_UPDATE_INPUT_POS, OnUpdateInputPosition)
  PIPE_MSG_HANDLE(NOMIME_IPC_START_MAINTENANCE, OnStartMaintenance)
  PIPE_MSG_HANDLE(NOMIME_IPC_END_MAINTENANCE, OnEndMaintenance)
  PIPE_MSG_HANDLE(NOMIME_IPC_COMMIT_COMPOSITION, OnCommitComposition)
  PIPE_MSG_HANDLE(NOMIME_IPC_CLEAR_COMPOSITION, OnClearComposition);
  PIPE_MSG_HANDLE(NOMIME_IPC_SELECT_CANDIDATE_ON_CURRENT_PAGE,
                  OnSelectCandidateOnCurrentPage);
  PIPE_MSG_HANDLE(NOMIME_IPC_HIGHLIGHT_CANDIDATE_ON_CURRENT_PAGE,
                  OnHighlightCandidateOnCurrentPage);
  PIPE_MSG_HANDLE(NOMIME_IPC_CHANGE_PAGE, OnChangePage);
  PIPE_MSG_HANDLE(NOMIME_IPC_TRAY_COMMAND, OnCommand);
