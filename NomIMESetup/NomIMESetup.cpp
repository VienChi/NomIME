// NomIMESetup.cpp : main source file for NomIMESetup.exe
//
...
static int CustomInstall(bool installing) {
  ...
  if (!_has_installed)
    if (0 != install(vi, silent))
      return 1;

  if (user_dir.empty()) {
    // default user dir %APPDATA%\NomIME
  ...
  ret = SetRegKeyValue(HKEY_CURRENT_USER, KEY, L"Vi", (vi ? 1 : 0),
                       REG_DWORD, false);
  if (FAILED(HRESULT_FROM_WIN32(ret))) {
    MSG_BY_IDS(IDS_STR_ERR_WRITE_VI, IDS_STR_INSTALL_FAILED,
               MB_ICONERROR | MB_OK);
    return 1;
  }
  if (_has_installed) {
    ...
    std::thread th([dir]() {
      ShellExecuteW(NULL, NULL, (dir + L"\\NomIMEServer.exe").c_str(), L"/q",
                    NULL, SW_SHOWNORMAL);
      Sleep(500);
      ShellExecuteW(NULL, NULL, (dir + L"\\NomIMEServer.exe").c_str(), L"",
                    NULL, SW_SHOWNORMAL);
      Sleep(500);
      ShellExecuteW(NULL, NULL, (dir + L"\\NomIMEDeployer.exe").c_str(),
...
static int Run(LPTSTR lpCmdLine) {
  ...
  if (!wcscmp(L"/?", lpCmdLine) || !wcscmp(L"/help", lpCmdLine)) {
    ...
    if (LoadString(GetModuleHandle(NULL), IDS_STR_HELP, msg,
      ...
      MessageBox(NULL, msg, L"NomIMESetup", MB_ICONINFORMATION | MB_OK);
    } else {
      MessageBox(
          NULL,
          L"Usage: NomIMESetup.exe [options]\n"
          L"/? or /help    - Show this help message\n"
          L"/u             - Uninstall NomIME\n"
          L"/i             - Install NomIME\n"
          L"/s             - Install NomIME (Simplified Chinese)\n"
          L"/v             - Install NomIME (Vietnamese)\n"
          L"/ls            - Set NomIME language to Simplified Chinese\n"
          L"/lt            - Set NomIME language to Traditional Chinese\n"
          L"/le            - Set NomIME language to English\n"
          ...
          L"NomIMESetup", MB_ICONINFORMATION | MB_OK);
    }
    return 0;
  }
  ...
  if (auto res = GetParamByPrefix(lpCmdLine, L"/userdir:")) {
    return SetRegKeyValue(HKEY_CURRENT_USER, L"Software\\SinoNom\\nomime",
