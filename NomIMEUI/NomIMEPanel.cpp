...
NomIMEPanel::NomIMEPanel(nomime::UI& ui)
    : m_layout(NULL),
      m_ctx(ui.ctx()),
      m_octx(ui.octx()),
      m_status(ui.status()),
...
void NomIMEPanel::_ResizeWindow() {
  CDCHandle dc = GetDC();
...
void NomIMEPanel::_CreateLayout() {
  if (m_layout != NULL)
    delete m_layout;
...
void NomIMEPanel::_InitFontRes(bool forced) {
  HMONITOR hMonitor = MonitorFromRect(m_inputPos, MONITOR_DEFAULTTONEAREST);
  UINT dpiX = 96, dpiY = 96;
  if (hMonitor)
    GetDpiForMonitor(hMonitor, MDT_EFFECTIVE_DPI, &dpiX, &dpiY);
...
void NomIMEPanel::_CaptureRect(CRect& rect) {
...
LRESULT NomIMEPanel::OnMouseActivate(UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam,
                                     BOOL& bHandled) {
  bHandled = true;
...
LRESULT NomIMEPanel::OnMouseWheel(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled) {
...
LRESULT NomIMEPanel::OnLeftClickedUp(UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam,
                                     BOOL& bHandled) {
  if (hide_candidates) {
...
LRESULT NomIMEPanel::OnLeftClickedDown(UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam,
                                       BOOL& bHandled) {
  if (hide_candidates) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < m_candidateCount && i < MAX_CANDIDATES_COUNT; ++i) {
      ...
      if (rect.PtInRect(point)) {
        ...
        ::SetTimer(m_hWnd, AUTOREV_TIMER, 1000, &NomIMEPanel::OnTimer);
        bHandled = true;
        return 0;
      }
    }
...
UINT_PTR NomIMEPanel::ptimer = 0;
VOID CALLBACK NomIMEPanel::OnTimer(_In_ HWND hwnd,
                                   _In_ UINT uMsg,
                                   _In_ UINT_PTR idEvent,
                                   _In_ DWORD dwTime) {
  ::KillTimer(hwnd, idEvent);
  NomIMEPanel* self = (NomIMEPanel*)ptimer;
  ptimer = 0;
  if (self) {
    self->bar_scale_ = 1.0;
    self->RedrawWindow();
...
LRESULT NomIMEPanel::OnMouseMove(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
...
LRESULT NomIMEPanel::OnMouseLeave(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled) {
  m_hoverIndex = -1;
...
void NomIMEPanel::_HighlightText(CDCHandle& dc,
                                 const CRect& rc,
...
bool NomIMEPanel::_DrawPreedit(const Text& text,
                               CDCHandle dc,
                               const CRect& rc) {
  bool drawn = false;
  ...
  if (!t.empty()) {
    nomime::TextRange range = m_layout->GetPreeditRange();
...
bool NomIMEPanel::_DrawPreeditBack(const Text& text,
                                   CDCHandle dc,
                                   const CRect& rc) {
  bool drawn = false;
  std::wstring const& t = text.str;
...
bool NomIMEPanel::_DrawCandidates(CDCHandle& dc, bool back) {
  bool drawn = false;
...
// draw client area
void NomIMEPanel::DoPaint(CDCHandle dc) {
...
void NomIMEPanel::RedrawWindow() {
  HDC hdc = GetDC();
  DoPaint(hdc);
  ReleaseDC(hdc);
}

void NomIMEPanel::_LayerUpdate(const CRect& rc, CDCHandle dc) {
  HDC ScreenDC = ::GetDC(NULL);
  CRect rect;
  GetWindowRect(&rect);
...
LRESULT NomIMEPanel::OnCreate(UINT uMsg,
                              WPARAM wParam,
                              LPARAM lParam,
                              BOOL& bHandled) {
  m_mouse_entry = false;
...
LRESULT NomIMEPanel::OnDestroy(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               BOOL& bHandled) {
  m_hoverIndex = -1;
...
LRESULT NomIMEPanel::OnDpiChanged(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled) {
  Refresh();
...
void NomIMEPanel::MoveTo(RECT const& rc) {
  if (!m_layout)
...
void NomIMEPanel::_RepositionWindow(const bool& adj) {
  RECT rcWorkArea;
...
void NomIMEPanel::_TextOut(const CRect& rc,
