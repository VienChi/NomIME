...
void nomime::StandardLayout::GetTextSizeDW(
...
CSize StandardLayout::GetPreeditSize(CDCHandle dc,
                                     const nomime::Text& text,
                                     ComPtr<IDWriteTextFormat1> pTextFormat,
                                     PDWR pDWR) {
  const std::wstring& preedit = text.str;
  const std::vector<nomime::TextAttribute>& attrs = text.attributes;
  CSize size(0, 0);
  if (!preedit.empty()) {
    nomime::TextRange range;
    for (size_t j = 0; j < attrs.size(); ++j)
      if (attrs[j].type == nomime::HIGHLIGHTED)
...
bool nomime::StandardLayout::_IsHighlightOverCandidateWindow(CRect& rc,
                                                             CDCHandle& dc) {
...
void nomime::StandardLayout::_PrepareRoundInfo(CDCHandle& dc) {
...
void StandardLayout::UpdateStatusIconLayout(int* width, int* height) {
  ...
  // and the status icon rule 3. status icon is right aligned in NomIMEPanel,
...
bool StandardLayout::IsInlinePreedit() const {
  return _style.inline_preedit &&
         (_style.client_caps & nomime::INLINE_PREEDIT_CAPABLE) != 0 &&
