...
#define NOMIME_IPC_WINDOW L"NomIMEIPCWindow_1.0"
#define NOMIME_IPC_PIPE_NAME L"NomIMENamedPipe"

#define NOMIME_IPC_METADATA_SIZE 1024
#define NOMIME_IPC_BUFFER_SIZE (4 * 1024)
#define NOMIME_IPC_BUFFER_LENGTH (NOMIME_IPC_BUFFER_SIZE / sizeof(WCHAR))
#define NOMIME_IPC_SHARED_MEMORY_SIZE \
  (sizeof(PipeMessage) + NOMIME_IPC_BUFFER_SIZE)

enum NOMIME_IPC_COMMAND {
  NOMIME_IPC_ECHO = (WM_APP + 1),
  NOMIME_IPC_START_SESSION,
  NOMIME_IPC_END_SESSION,
  NOMIME_IPC_PROCESS_KEY_EVENT,
  NOMIME_IPC_SHUTDOWN_SERVER,
  NOMIME_IPC_FOCUS_IN,
  NOMIME_IPC_FOCUS_OUT,
  NOMIME_IPC_UPDATE_INPUT_POS,
  NOMIME_IPC_START_MAINTENANCE,
  NOMIME_IPC_END_MAINTENANCE,
  NOMIME_IPC_COMMIT_COMPOSITION,
  NOMIME_IPC_CLEAR_COMPOSITION,
  NOMIME_IPC_TRAY_COMMAND,
  NOMIME_IPC_SELECT_CANDIDATE_ON_CURRENT_PAGE,
  NOMIME_IPC_HIGHLIGHT_CANDIDATE_ON_CURRENT_PAGE,
  NOMIME_IPC_CHANGE_PAGE,
  NOMIME_IPC_LAST_COMMAND
};

namespace nomime {
struct PipeMessage {
  NOMIME_IPC_COMMAND Msg;
  DWORD wParam;
  DWORD lParam;
};
...
inline std::wstring GetPipeName() {
  ...
  pipe_name += L"\\";
  pipe_name += NOMIME_IPC_PIPE_NAME;
  return pipe_name;
}
}  // namespace nomime
