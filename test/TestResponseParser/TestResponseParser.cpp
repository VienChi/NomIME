...
void test_1() {
  ...
  std::wstring commit;
  nomime::Context ctx;
  nomime::Status status;
  nomime::ResponseParser parser(&commit, &ctx, &status);
  parser(resp, len);
...
void test_4() {
  ...
  nomime::TextAttribute attr0 = ctx.preedit.attributes[0];
  BOOST_TEST_EQ(nomime::HIGHLIGHTED, attr0.type);
  BOOST_TEST_EQ(0, attr0.range.start);
  BOOST_TEST_EQ(3, attr0.range.end);
  BOOST_TEST(ctx.aux.empty());
  nomime::CandidateInfo& c = ctx.cinfo;
