// TestNomIMEIPC.cpp : Defines the entry point for the console application.
//
...
// usage: TestNomIMEIPC.exe [/start | /stop | /console]
...
bool launch_server() {
  int ret = (int)ShellExecute(NULL, L"open", L"TestNomIMEIPC.exe", L"/start",
                              NULL, SW_NORMAL);
  if (ret <= 32) {
...
bool read_buffer(LPWSTR buffer, UINT length, LPWSTR dest) {
  ...
  bs.read(dest, NOMIME_IPC_BUFFER_LENGTH);
  return bs.good();
}
...
int console_main() {
  ...
  while (std::cin.good()) {
    ...
    if (!std::cin.good())
      break;
    bool eaten = client.ProcessKeyEvent(nomime::KeyEvent(ch, 0));
    std::cout << "server replies: " << eaten << std::endl;
    if (eaten) {
      WCHAR response[NOMIME_IPC_BUFFER_LENGTH];
...
int client_main() {
  ...
  bool eaten = client.ProcessKeyEvent(nomime::KeyEvent(L'a', 0));
  std::cout << "server replies: " << eaten << std::endl;
  if (eaten) {
    WCHAR response[NOMIME_IPC_BUFFER_LENGTH];
    bool ret = client.GetResponseData(
...
class TestRequestHandler : public nomime::RequestHandler {
 public:
  ...
  virtual BOOL ProcessKeyEvent(nomime::KeyEvent keyEvent,
                               UINT session_id,
                               EatLine eat) {
    std::cerr << "ProcessKeyEvent: " << session_id
              << " keycode: " << keyEvent.keycode << " mask: " << keyEvent.mask
              << std::endl;
    eat(std::wstring(L"Greeting=Hello, NomIME.\n"));
    return TRUE;
  }
...
int server_main() {
  ...
  nomime::Server server;
  // nomime::UI ui;
  // const std::unique_ptr<nomime::RequestHandler> handler(new
  // RimeWithNomIMEHandler(&ui));
  const std::unique_ptr<nomime::RequestHandler> handler(new TestRequestHandler);
