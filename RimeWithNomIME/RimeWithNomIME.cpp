...
NomIMESessionId _GenerateNewNomIMESessionId(SessionStatusMap sm, DWORD pid) {
  if (sm.empty())
    return (NomIMESessionId)(pid + 1);
  return (NomIMESessionId)(sm.rbegin()->first + 1);
}
...
RimeWithNomIMEHandler::RimeWithNomIMEHandler(UI* ui)
    : m_ui(ui),
      m_active_session(0),
      m_disabled(true),
...
RimeWithNomIMEHandler::~RimeWithNomIMEHandler() {
...
void RimeWithNomIMEHandler::_Setup() {
  ...
  rime_api->set_notification_handler(&RimeWithNomIMEHandler::OnNotify, this);
}

void RimeWithNomIMEHandler::Initialize() {
  ...
  if (rime_api->config_open("nomime", &config)) {
    if (m_ui) {
...
void RimeWithNomIMEHandler::Finalize() {
  m_active_session = 0;
  m_disabled = true;
  m_session_status_map.clear();
...
DWORD RimeWithNomIMEHandler::FindSession(NomIMESessionId ipc_id) {
  if (m_disabled)
    return 0;
...
DWORD RimeWithNomIMEHandler::AddSession(LPWSTR buffer, EatLine eat) {
  if (m_disabled) {
  ...
  NomIMESessionId ipc_id =
      _GenerateNewNomIMESessionId(m_session_status_map, m_pid);
...
DWORD RimeWithNomIMEHandler::RemoveSession(NomIMESessionId ipc_id) {
  if (m_ui)
    m_ui->Hide();
  if (m_disabled)
    return 0;
...
void RimeWithNomIMEHandler::UpdateColorTheme(BOOL darkMode) {
  RimeConfig config = {NULL};
  if (rime_api->config_open("nomime", &config)) {
    if (m_ui) {
      _UpdateUIStyle(&config, m_ui, true);
...
BOOL RimeWithNomIMEHandler::ProcessKeyEvent(KeyEvent keyEvent,
                                            NomIMESessionId ipc_id,
                                            EatLine eat) {
...
void RimeWithNomIMEHandler::CommitComposition(NomIMESessionId ipc_id) {
...
void RimeWithNomIMEHandler::ClearComposition(NomIMESessionId ipc_id) {
...
void RimeWithNomIMEHandler::SelectCandidateOnCurrentPage(
    size_t index,
    NomIMESessionId ipc_id) {
...
bool RimeWithNomIMEHandler::HighlightCandidateOnCurrentPage(
    size_t index,
    NomIMESessionId ipc_id,
    EatLine eat) {
...
bool RimeWithNomIMEHandler::ChangePage(bool backward,
                                       NomIMESessionId ipc_id,
                                       EatLine eat) {
...
void RimeWithNomIMEHandler::FocusIn(DWORD client_caps, NomIMESessionId ipc_id) {
...
void RimeWithNomIMEHandler::FocusOut(DWORD param, NomIMESessionId ipc_id) {
...
void RimeWithNomIMEHandler::UpdateInputPosition(RECT const& rc,
                                                NomIMESessionId ipc_id) {
...
std::string RimeWithNomIMEHandler::m_message_type;
std::string RimeWithNomIMEHandler::m_message_value;
std::string RimeWithNomIMEHandler::m_message_label;
std::string RimeWithNomIMEHandler::m_option_name;
std::mutex RimeWithNomIMEHandler::m_notifier_mutex;

void RimeWithNomIMEHandler::OnNotify(void* context_object,
                                     uintptr_t session_id,
                                     const char* message_type,
                                     const char* message_value) {
  // may be running in a thread when deploying rime
  RimeWithNomIMEHandler* self =
      reinterpret_cast<RimeWithNomIMEHandler*>(context_object);
...
void RimeWithNomIMEHandler::_ReadClientInfo(NomIMESessionId ipc_id,
                                            LPWSTR buffer) {
  std::string app_name;
  // parse request text
  wbufferstream bs(buffer, NOMIME_IPC_BUFFER_LENGTH);
  std::wstring line;
  while (bs.good()) {
    std::getline(bs, line);
    if (!bs.good())
  ...
  // show soft cursor on nomime panel but not inline
  rime_api->set_option(session_id, "soft_cursor", Bool(!inline_preedit));
}

void RimeWithNomIMEHandler::_GetCandidateInfo(CandidateInfo& cinfo,
                                              RimeContext& ctx) {
...
void RimeWithNomIMEHandler::StartMaintenance() {
  m_session_status_map.clear();
  Finalize();
  _UpdateUI(0);
}

void RimeWithNomIMEHandler::EndMaintenance() {
  if (m_disabled) {
    Initialize();
    _UpdateUI(0);
  }
...
void RimeWithNomIMEHandler::SetOption(NomIMESessionId ipc_id,
...
void RimeWithNomIMEHandler::OnUpdateUI(std::function<void()> const& cb) {
  _UpdateUICallback = cb;
}

bool RimeWithNomIMEHandler::_IsDeployerRunning() {
...
void RimeWithNomIMEHandler::_UpdateUI(NomIMESessionId ipc_id) {
  // if m_ui nullptr, _UpdateUI meaningless
  if (!m_ui)
    return;

  Status& nomime_status = m_ui->status();
  Context nomime_context;

  RimeSessionId session_id = to_session_id(ipc_id);

  if (ipc_id == 0)
    nomime_status.disabled = m_disabled;

  _GetStatus(nomime_status, ipc_id, nomime_context);
  ...
  if (!_ShowMessage(nomime_context, nomime_status)) {
    m_ui->Hide();
    m_ui->Update(nomime_context, nomime_status);
  }
...
void RimeWithNomIMEHandler::_LoadSchemaSpecificSettings(
    NomIMESessionId ipc_id,
  ...
  const auto update_color_scheme = [&]() {
    ...
    if (rime_api->config_begin_map(
      ...
      RimeConfig nomimeconfig;
      if (rime_api->config_open("nomime", &nomimeconfig)) {
        _UpdateUIStyleColor(&nomimeconfig, style, color_name);
        rime_api->config_close(&nomimeconfig);
      }
    }
  };
  const char* key =
  ...
  // load schema icon start
  {
    const auto load_icon = [](RimeConfig& config, const char* key1,
                              const char* key2) {
      const auto user_dir = NomIMEUserDataPath();
      const auto shared_dir = NomIMESharedDataPath();
      const int BUF_SIZE = 255;
      char buffer[BUF_SIZE + 1] = {0};
...
void RimeWithNomIMEHandler::_LoadAppInlinePreeditSet(NomIMESessionId ipc_id,
...
bool RimeWithNomIMEHandler::_ShowMessage(Context& ctx, Status& status) {
  std::lock_guard<std::mutex> lock(m_notifier_mutex);
...
bool RimeWithNomIMEHandler::_Respond(NomIMESessionId ipc_id, EatLine eat) {
  std::wstring body;
  body.reserve(4096);
...
void RimeWithNomIMEHandler::_UpdateShowNotifications(RimeConfig* config,
                                                     bool initialize) {
...
void RimeWithNomIMEHandler::_GetStatus(Status& stat,
                                       NomIMESessionId ipc_id,
                                       Context& ctx) {
  SessionStatus& session_status = get_session_status(ipc_id);
  RimeSessionId session_id = session_status.session_id;
...
void RimeWithNomIMEHandler::_GetContext(Context& nomime_context,
                                        RimeSessionId session_id) {
  RIME_STRUCT(RimeContext, ctx);
  if (rime_api->get_context(session_id, &ctx)) {
    if (ctx.composition.length > 0) {
      nomime_context.preedit.str = u8tow(ctx.composition.preedit);
      if (ctx.composition.sel_start < ctx.composition.sel_end) {
        ...
        nomime_context.preedit.attributes.push_back(attr);
      }
    }
    if (ctx.menu.num_candidates) {
      CandidateInfo& cinfo(nomime_context.cinfo);
      _GetCandidateInfo(cinfo, ctx);
    }
    rime_api->free_context(&ctx);
  }
...
void RimeWithNomIMEHandler::_UpdateInlinePreeditStatus(NomIMESessionId ipc_id) {
  if (!m_ui)
    return;
  SessionStatus& session_status = get_session_status(ipc_id);
  RimeSessionId session_id = session_status.session_id;
