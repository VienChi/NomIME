...
function update_changelog {
  ...
  $commits = git log "$old_tag..HEAD" --pretty=format:"%s ([%an](https://github.com/rime/nomime/commit/%H))";
  # group commit logs
  $groupedCommits = @{
  ...
  $contentAdd += "## [$new_tag](https://github.com/rime/nomime/compare/$old_tag...$new_tag)($currentDateTime)`n" ;
...
function update_xml {
  ...
  replace_str -filePath "$filePath" `
    ...
    -pat_replace "http://bogo.hannom.org/release/nomime/"
  replace_str -filePath "$filePath" `
...
# test if cwd is nomime root
function is_nomime_root() {
  return (Test-Path -Path "nomime.sln") -and (Test-Path -Path ".git")
}
###############################################################################
# program now started
if ($h -or (-not $tag)) {
...
if (-not (is_nomime_root)) {
  Write-Host "Current directory is: $(Get-Location), it's not the nomime root directory"
  Write-Host "please run ``update/bump_version.ps1`` with parameters under the nomime root in Powershell";
  Write-Host "or run ``pwsh update/bump_version.ps1`` with parameters under nomime root in shell if yor're running Mac OS or Linux"
  exit 1;
}
