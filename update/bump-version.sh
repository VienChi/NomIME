...
update_changelog() {
  ...
  contentAdd+="## [$new_tag](https://github.com/rime/nomime/compare/$old_tag...$new_tag)($currentDateTime)"$'\n'
...
update_xml() {
  ...
  replace_str "$1" "\(<sparkle:releaseNotesLink>\)[^<]*\(</sparkle:releaseNotesLink>\)" \
    "\1http://bogo.hannom.org/release/nomime/\2"
  # update version tag
...
# check if the script run under the nomime root directory
if ! [[ -f nomime.sln && -d .git ]]; then
  echo "Current directory is: $(pwd), it's not the nomime root directory"
  echo "please run 'update/bump_version.sh' with parameters under the nomime root in bash"
  exit 1
fi
