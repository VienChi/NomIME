...
includes("NomIMEIPC", "NomIMEUI", "NomIMETSF")

if is_arch("x64") or is_arch("x86") then
  includes("RimeWithNomIME", "NomIMEIPCServer", "NomIMEServer", "NomIMEDeployer")
end

if is_arch("x86") then
  includes("NomIMESetup")
end

if is_mode("debug") then
  includes("test/TestNomIMEIPC")
...
rule("use_nomimeconstants")
  on_load(function(target)
    function check_include_nomime_constants_in_dir(dir)
      local files = os.files(path.join(dir, "**.h"))
      table.join2(files, os.files(path.join(dir, "**.cpp")))
      for _, file in ipairs(files) do
        local content = io.readfile(file)
        if content:find('#include%s+"NomIMEConstants%.h"') or content:find('#include%s+<NomIMEConstants%.h>') then
          return true
        end
      end
      return false
    end
    if check_include_nomime_constants_in_dir(target:scriptdir()) then
      target:add("rcflags", {
